...
order by IP.SeqNo";
        ...
        public IEnumerable<ProcessModel> GetProcessList(int CompanyId)
        {
            using (IDbConnection conn = new SqlConnection(ErpGlobal.DBCONNECTIONSTRING))
            {
                string sqlQuery = @"Select PNM.process_Name_id ProcessId,PNM.Process_Name ProcessName,PNM.ShortName from 
Process_name_Master PNM Where PNM.MasterCompanyId=@CompanyId 
